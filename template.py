...
list_of_files = [
   ...
   f"src/{package_name}/exception__init__.py",
   f"src/{package_name}/logger__init__.py",
   f"src/{package_name}/utils/__init__.py",
   f"src/{package_name}/pipeline/__init__.py",
   "notebooks/data_collections.ipynb",
   "notebooks/ocr.ipynb",
   "requirements.txt",
   "app.py"
...
for file_path in list_of_files:
    ...
    if (not os.path.exists(file_path)) or (os.path.getsize(file_path)==0):
        with open(file_path,"w") as f:
            pass #Create an empty file and and do nothing
